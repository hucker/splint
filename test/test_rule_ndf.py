...
df libraries work, but that the splint interface to narwhals is working with the ideal being that
splint has NO dependency on dataframe libraries and only depends upon narwhals.
...
from src.splint.rule_ndf import extended_bool
from src.splint.rule_ndf import rule_ndf_columns_check, rule_ndf_pf_columns, rule_validate_ndf_values_by_col
...
def test_rule_ndf_pf_columns_enabled(request, df_fixture_name, expected_status, expected_length):
    ...
    s_func1 = SplintFunction(check_narwhals_pf_frame)
