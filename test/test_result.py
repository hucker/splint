...
def results():
    pkg = splint.SplintPackage(folder="./pkg_result")
    chk = splint.SplintChecker(packages=pkg, auto_setup=True)
    return chk.run_all()
...
def test_no_info_filter(results):
    ...
    no_info = [r for r in results if splint_result.remove_info(r)]
    assert len(no_info) == 6


def test_warn_is_fail_filter(results):
    """ Test to verify the warn_as_fail filter function """
    warn_is_fail = [r for r in results if splint_result.warn_as_fail(r)]
...
def test_group_by_tags_ruid(results):
    ...
    grouped_results = splint_result.group_by(results, ['tag', 'ruid'])
    assert len(grouped_results) == 3
    assert len(grouped_results['tag1']) == 3
...
def test_group_by_ruids(results):
    ...
    r_grouped_results = splint_result.group_by(results, ['ruid'])
